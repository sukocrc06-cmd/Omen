...
def main():
    ...
    print(f"Güncel sinyal: {rec['signal']}  (skor={rec['score']}, güven=%{rec['confidence_pct']})")

    state_path = os.path.join(args.state_dir, STATE_FILE_TEMPLATE.format(
        ticker=args.ticker.replace(".", "_"), interval=interval))
    prev_state = load_state(state_path)
    changed = args.test or (prev_state is None) or (prev_state.get("signal") != rec["signal"])

    if not changed:
        print("Sinyal önceki bildirimden bu yana değişmedi -> bildirim gönderilmiyor.")
        return

    subject, body = build_message(args.ticker, interval, rec)
    ...
    if tg_cfg.get("enabled") and tg_cfg.get("bot_token") and tg_cfg.get("chat_id"):
        ...
        print("Telegram bildirimi gönderildi." if ok else "Telegram bildirimi GÖNDERİLEMEDİ.")
        sent_any = sent_any or ok

    email_cfg = cfg.get("email", {})
    if email_cfg.get("enabled"):
        ok = send_email(email_cfg, subject, body)
        print("E-posta gönderildi." if ok else "E-posta GÖNDERİLEMEDİ.")
        sent_any = sent_any or ok

    if not sent_any:
        print("UYARI: Ne Telegram ne e-posta gönderilebildi (ya devre dışı ya da hata oluştu). "
